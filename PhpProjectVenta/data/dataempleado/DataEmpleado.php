...
class DataEmpleado {
    ...
    public function buscarEmpleado($empleadoid) {
        ...
        if ($this->conexion->crearConexion()->set_charset('utf8') == true) {
            ...
            $array = array();
            $buscarempleado = $this->conexion->crearConexion()->query("SELECT e.empleadoid, p.personanombre, p.personaapellido1,
                p.personaapellido2, p.personatelefono, p.personacorreo, e.empleadofechaingreso, e.tipoempleado, e.empleadocontrasenia,
                e.empleadobanco, e.empleadocuentabancaria, e.empleadoestadocivil, e.empleadoedad,
                e.empleadosexo, v.vehiculomarca, el.empleadolicenciatipo, el.empleadolicenciavigencia,
                z.zonanombre
                FROM tbempleados e
                ...
                WHERE e.empleadoid=$empleadoid AND e.empleadoestado=1;");
